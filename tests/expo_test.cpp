...
namespace
{
    ...
    {
        rx::testing::tree_matcher<CharT> tree{ pattern , rx::detail::parser_flags{ .enable_possessive=true, .enable_backrefs=true } };
...
// static_assert(tree_match("(ab(cd))\\1\\2", "abcdabcdcd"));       // TODO: fix capture numbering order
// static_assert(not tree_match("(ab(cd))\\1\\2", "abcdcdabcd"));
// static_assert(tree_match("((ab|e\\3)(c*)d){2}", "abccdeccd"))
static_assert(tree_match("((a)(b)\\g2\\g1){2}", "abbaabba"));
