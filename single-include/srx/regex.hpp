...
private:
    ...
    using maybe_generation_t = maybe_type_t<generation::enabled, generation>;
    ...
    static constexpr void register_operations(context<I, X> ctx, const I it)
    {
        if constexpr (Blk != tdfa::no_transition_regops and X != 0)
        {
            ...
            if constexpr (generation::enabled)
            {
                ...
                {
                    if constexpr (op.is_copy)
                        ctx.osr.generations.reg[op.dst] = ctx.osr.generations.reg[op.cpy_src];
                    else if constexpr (op.set_val)
                        ctx.osr.generations.reg[op.dst] = ctx.osr.generations.current;
                    else
                        ctx.osr.generations.reg[op.dst] = 0;
                }
            }
        }
    }
...
struct p1306dfa<Info>::iterated_result
{
    ...
    friend p1306dfa<Info>::context<I, 2>;
...
template<typename R>
concept static_regex_match_view_like = is_template_instantiation_of_type(^^std::ranges::range_value_t<R>, ^^static_match_results);

template<typename Regex>
concept static_regex_like = is_template_instantiation_of_type(^^Regex, ^^static_regex);
